...
int aws_event_stream_message_to_debug_str(FILE *fd, const struct aws_event_stream_message *message) {
    ...
    for (uint16_t i = 0; i < headers_count; ++i) {
        ...
        if (header->header_value_type == AWS_EVENT_STREAM_HEADER_BOOL_FALSE) {
            ...
            if (header->header_value_type == AWS_EVENT_STREAM_HEADER_UUID) {
                ...
                struct aws_byte_cursor cursor = aws_byte_cursor_from_buf(&to_encode);
                aws_base64_encode(&cursor, &encode_output);
            } else {
                struct aws_byte_buf to_encode =
    ...
    struct aws_byte_cursor cursor = aws_byte_cursor_from_buf(&payload_buffer);
    aws_base64_encode(&cursor, &encoded_payload_buffer);
